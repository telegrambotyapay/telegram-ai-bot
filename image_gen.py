...
def generate_gemini_image(prompt: str) -> bytes:
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 403:
                raise ImageGenError(
                    "Gemini 403 hatası: API key yanlış olabilir ya da bu modele erişimin yok. "
                    "GOOGLE_AI_STUDIO_API_KEY'i kontrol et."
                )
            if resp.status_code == 404:
                raise ImageGenError("Gemini görsel modeli bulunamadı, model adı değişmiş olabilir.")
            if resp.status_code == 429 or resp.status_code >= 500:
                last_error = f"{resp.status_code} {resp.reason}"
                logger.warning(f"Gemini görsel yoğunluk/sunucu hatası, {attempt + 1}. deneme...")
    ...
    raise ImageGenError(f"Gemini şu an geçici olarak yanıt veremiyor. ({last_error})")
...
def analyze_image_groq(image_bytes: bytes, instruction: str = None, mime_type: str = "image/jpeg") -> str:
    """
    Groq'un görme özellikli modeliyle (qwen3.6-27b, preview) görsel analiz yapar.
    Not: Bu model Groq'ta hâlâ 'preview' aşamasında, üretim için resmi olarak
    önerilmiyor ama Gemini'ye alternatif/yedek olarak kullanışlı.
    """
    if not config.GROQ_API_KEY:
        raise ImageGenError("GROQ_API_KEY tanımlı değil.")
    ...
    try:
        ...
        if not text or not text.strip():
            raise ImageGenError("Groq cevabında metin bulunamadı.")
        return text
    except ImageGenError:
        raise
    except Exception as e:
        raise ImageGenError(f"Groq görsel analizi başarısız: {e}") from e


def analyze_image(image_bytes: bytes, instruction: str = None, mime_type: str = "image/jpeg") -> str:
    """Önce Groq'u dener (varsayılan), o başarısız olursa Gemini'ye düşer."""
    try:
        return analyze_image_groq(image_bytes, instruction, mime_type)
    except ImageGenError as e:
        logger.warning(f"Groq görsel analizi başarısız ({e}), Gemini deneniyor...")
        try:
            return analyze_image_gemini(image_bytes, instruction, mime_type)
        except ImageGenError as e2:
            raise ImageGenError(f"Hem Groq hem Gemini başarısız oldu. Groq: {e} | Gemini: {e2}") from e2
...
def analyze_image_gemini(image_bytes: bytes, instruction: str = None, mime_type: str = "image/jpeg") -> str:
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 404:
                raise ImageGenError("Gemini modeli bulunamadı, model adı değişmiş olabilir.")
            if resp.status_code == 429 or resp.status_code >= 500:
                last_error = f"{resp.status_code} {resp.reason}"
                logger.warning(f"Gemini analiz yoğunluk/sunucu hatası, {attempt + 1}. deneme...")
                time.sleep(3 * (attempt + 1))
                continue
            resp.raise_for_status()
            data = resp.json()
