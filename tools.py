...
def fetch_and_extract_url(url: str) -> str:
    """Bir web sayfasını ScrapingBee ile çeker, okunabilir metnini çıkarır."""
    if not config.SCRAPINGBEE_API_KEY:
        raise ToolError("SCRAPINGBEE_API_KEY tanımlı değil.")
    ...
    try:
        ...
        content = "\n".join(lines)
        if not content:
            raise ToolError("Sayfadan okunabilir bir içerik çıkarılamadı.")
        return content[:6000]
    except ToolError:
        raise
    except Exception as e:
        raise ToolError(f"Sayfa okunamadı: {e}") from e


def analyze_face(image_bytes: bytes) -> str:
    """Face++ ile bir fotoğraftaki yüzleri analiz eder (yaş, cinsiyet, duygu)."""
    if not (config.FACEPP_API_KEY and config.FACEPP_API_SECRET):
        raise ToolError("FACEPP_API_KEY ve FACEPP_API_SECRET ikisi de tanımlı olmalı.")
    try:
        ...
        if data.get("error_message"):
            raise ToolError(data["error_message"])
        faces = data.get("faces", [])
        if not faces:
            raise ToolError("Fotoğrafta yüz tespit edilemedi.")
        lines = [f"😀 {len(faces)} yüz tespit edildi:\n"]
        for i, face in enumerate(faces, 1):
            attr = face.get("attributes", {})
            age = attr.get("age", {}).get("value", "?")
            gender_raw = attr.get("gender", {}).get("value", "?")
            gender = "Erkek" if gender_raw == "Male" else "Kadın" if gender_raw == "Female" else gender_raw
            emotion = attr.get("emotion", {})
            top_emotion = max(emotion.items(), key=lambda x: x[1])[0] if emotion else "?"
            emotion_tr = {
                "happiness": "mutlu", "sadness": "üzgün", "anger": "kızgın",
                "surprise": "şaşkın", "fear": "korkmuş", "disgust": "tiksinmiş",
                "neutral": "nötr",
            }.get(top_emotion, top_emotion)
            lines.append(f"{i}. Yaş: ~{age}, Cinsiyet: {gender}, Baskın duygu: {emotion_tr}")
        return "\n".join(lines)
    except ToolError:
        raise
    except Exception as e:
        raise ToolError(f"Yüz analizi başarısız: {e}") from e
